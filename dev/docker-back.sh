...
-p 8200:8200 \
docker.elastic.co/apm/apm-server:7.8.0 -strict.perms=false
...
sudo docker run -d --restart unless-stopped --name=metricbeat --user=root \
  ...
  -v /:/hostfs:ro \
  --network host \
  docker.elastic.co/beats/metricbeat:7.8.0 metricbeat -e -system.hostfs=/hostfs -strict.perms=false 
